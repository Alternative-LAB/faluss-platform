...
/** Exact Me-owned AN-01 catalog provider; registration never accepts or emits an event. */
final class Faluss_Link_Events_Catalog {
    ...
    public static function register_provider() {
        ...
        if ( is_wp_error( $identity ) || 'me-node' !== ( $identity['node_id'] ?? null ) || 'faluss-me' !== ( $identity['app_key'] ?? null ) || 'https://faluss.me' !== ( $identity['origin'] ?? null ) ) {
            return false;
        }
    ...
    public static function provide( $context ) {
        ...
        if ( self::$conflict || ! class_exists( 'Faluss_Events_Catalog_Validator' ) || ! self::valid_context( $context ) || ! self::valid_catalog( $catalog ) || ! Faluss_Events_Catalog_Validator::validate_federation_payload( $catalog, $contract, $context ) ) {
            return new WP_Error( 'faluss_link_events_catalog_refused' );
        }
    ...
    public static function descriptor() {
        return array(
            'owner_app_key' => 'faluss-me',
            'capability_key' => 'faluss-me.events',
            'catalog_version' => '1.0.0',
            'catalog_provider' => array( __CLASS__, 'provide' ),
            'cap_manifest_provider' => array( 'Faluss_Link_Manifest', 'provide' ),
        );
    }
    ...
    public static function catalog() {
        return array(
            ...
            'app_key' => 'faluss-me',
            'owner' => 'faluss-me',
            'owner_engine' => 'faluss-link',
            'capability_key' => 'faluss-me.events',
            'capability_interface' => 'event_source',
            'event_types' => array(
                self::event_definition( 'faluss-me.card.viewed', 'faluss-me.card-viewed', 'forbidden', array() ),
                self::event_definition( 'faluss-me.link.clicked', 'faluss-me.link-clicked', 'required', array( 'link' ) ),
                self::event_definition( 'faluss-me.collection.opened', 'faluss-me.collection-opened', 'required', array( 'collection' ) ),
            ),
    ...
    private static function event_definition( $event_type, $document_type, $object_presence, $object_types ) {
        return array(
            ...
            'allowed_actor_types' => array( 'anonymous' ),
    ...
    private static function valid_local_contract() {
        ...
        $manifest_context['parameters'] = array( 'app_key' => 'faluss-me', 'requested_manifest_version' => '1.0.0' );
        return self::valid_catalog( self::catalog() )
            && Faluss_Events_Catalog_Validator::validate_against_manifest( self::catalog(), Faluss_Link_Manifest::manifest(), array( 'document_type' => 'faluss.app-capability-manifest', 'contract_version' => '1.0.0' ), $manifest_context )
    ...
    private static function local_context( $operation ) {
        return array(
            'operation' => $operation,
            'parameters' => array( 'owner_app_key' => 'faluss-me', 'capability_key' => 'faluss-me.events', 'catalog_version' => '1.0.0' ),
            'subject_context' => null,
            'sender' => array( 'node_id' => 'me-node', 'app_key' => 'faluss-me' ),
    ...
    private static function valid_context( $context ) {
        return self::exact_keys( $context, array( 'operation', 'parameters', 'subject_context', 'sender', 'recipient' ) )
            ...
            && array( 'owner_app_key' => 'faluss-me', 'capability_key' => 'faluss-me.events', 'catalog_version' => '1.0.0' ) === $context['parameters']
            && self::sender( $context['sender'] )
            && self::exact_keys( $context['recipient'], array( 'node_id', 'app_key' ) )
            && array( 'node_id' => 'me-node', 'app_key' => 'faluss-me' ) === $context['recipient'];
    }
