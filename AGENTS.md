...
- Merge into `main` only through a reviewed pull request after all required checks pass. The author must not approve their own pull request.
- Changes to `AGENTS.md`, `.github/`, or the governance documentation follow the same pull request and review rules; a change to a check must not validate itself using the proposed version of that check.
