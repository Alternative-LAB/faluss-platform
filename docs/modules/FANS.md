...
Les tests de la PR #53 démontrent la reconnaissance du rôle et l'isolation de ses hooks. Ceux des PR SSO, profils et followers ajoutent des preuves unitaires. Une recette locale jetable WordPress/MariaDB a vérifié les tables, la création/liaison SSO directe, les permissions REST des profils et la fermeture des routes après retrait des flags ; l'échange réseau avec Me, le navigateur, les routes Followers en HTTP et les courses réelles restent à tester. Aucun test de vente n'existe encore puisqu'aucun service de vente n'est implémenté ; ce verrou devra être prouvé par tests serveur et API dans le lot commerce.
