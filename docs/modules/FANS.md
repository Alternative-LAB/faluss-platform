...
| Followers | Implémenté dans cette PR dépendante, opt-in, non activé sur un site réel | Suivi/retrait idempotents d'un profil actif par un membre SSO lié ; seul le nombre est public. Blocage et signalements restent à construire. Voir [FANS-FOLLOWERS.md](FANS-FOLLOWERS.md). |
| Publications, messagerie, store, commandes et droits | Contractuel seulement | Aucun de ces domaines, stockage, route, transaction, achat ou droit n'est implémenté ici. |
...
Les tests de la PR #53 démontrent la reconnaissance du rôle et l'isolation de ses hooks. Ceux des PR SSO, profils et followers ajoutent des preuves unitaires, sans recette WordPress/MariaDB ou réseau réelle. Aucun test de vente n'existe encore puisqu'aucun service de vente n'est implémenté ; ce verrou devra être prouvé par tests serveur et API dans le lot commerce.
