...
Les tests de la PR #53 démontrent la reconnaissance du rôle et l'isolation de ses hooks. Ceux du présent lot ajoutent des preuves unitaires du client SSO, sans recette WordPress/MariaDB ou réseau réelle. Aucun test de vente n'existe encore puisqu'aucun service de vente n'est implémenté ; ce verrou devra être prouvé par tests serveur et API dans le lot commerce.
