...
Le shell de l’onboarding et le bas du panneau restent ancrés au viewport de mise en page. Le clavier se superpose : `visualViewport` sert uniquement à calculer la zone visible et le défilement interne du champ actif. Le focus ne déclenche aucune expansion ; le bouton peut être recouvert, puis retrouve exactement sa place. Le document ne défile pas. La hauteur initiale mesure les contrôles, avec un plafond de 66 % et une réserve de 64 % pour Identité/Réseaux afin de garder une zone de saisie au-dessus du clavier. Les étapes peu chargées restent plus basses. Le geste volontaire agrandit d’abord le panneau vers le haut, puis permet le défilement intégral. Les champs ont une taille de 16 px, sans restriction du zoom volontaire. La couleur des boutons détermine côté serveur un texte noir ou blanc contrasté.
