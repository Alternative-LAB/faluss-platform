...
Le curseur est enregistré dans `onboarding_next_step` d'Identity. Les anciens curseurs `choice`, `identifier`, `studio`, `wizard_*` et `wizard_atomic_*` sont projetés vers un écran V3 sans écriture à la lecture. Le mode choisi est encodé dans `v3_identity_simple` ou `v3_identity_atomic` avant l'enregistrement des préférences Link. Chaque passage suivant, retour ou étape passée sauvegarde les données de l'étape et le curseur dans la transaction Link/Identity existante. La réservation du slug utilise la transaction Identity historique juste avant l'enregistrement de l'identité ; si celui-ci échoue, le slug reste réservé au même compte et peut être repris sans nouvelle identité.
